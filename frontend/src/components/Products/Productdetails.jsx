...
const ProductDetails = ({productId}) => {
    ...
    ).then(()=>{toast.success("Product added to the cart",{
        duration:1000,
    })}).finally(()=>{
                            ...
                            <div className="flex gap-2 mt-2">
                                {selectedProduct.sizes.map((size) => (
                                    <button key={size} onClick={() => setSelectedSize(size)}
                                        className={`w-8 h-8 rounded border-2 border-gray-200 ${selectedSize === size ? "bg-black text-white" : ""}`}>{size}</button>
                                ))}
                            </div>
                        </div>
                        <div className="mb-6">
                            <p className="text-gray-700">Quantity:</p>
                            <div className="flex itmes-center space-x-4 mt-2">
